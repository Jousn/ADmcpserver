...
begin
    ...
    begin
        ...
        begin
            ...
            // Add to board (new objects must not be wrapped in BeginModify/EndModify before registration)
...
begin
    ...
    try
        ...
        begin
            Component := FindPcbComponentByRefDes(Board, SourceCmps[i]);
    ...
    try
        ...
        try
            ...
            begin
                ...
                begin
                    CmpSrc := FindPcbComponentByRefDes(Board, SourceList.Get(i));
                    CmpDst := FindPcbComponentByRefDes(Board, DestList.Get(i));
        ...
        begin
            ...
            begin
                ...
                CmpSrc := FindPcbComponentByRefDes(Board, NameSrc);

                NameDst := DestList.Get(i);
                CmpDst := FindPcbComponentByRefDes(Board, NameDst);
                ...
                begin
                    ...
                    // Verified: BeginModify/EndModify + Layer property write — DesignReuse.pas L306-L325
                    CmpDst.Rotation := CmpSrc.Rotation;
                    CmpDst.Layer := CmpSrc.Layer;
                    CmpDst.x := CmpSrc.x;
                    CmpDst.y := CmpSrc.y;
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        begin
                                            ...
                                            // Rebuild polygon after net assignment to validate geometry
                                            Polygon.Rebuild;
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    if DstNetName <> '' then
                    begin
                        Net := FindNetByName(Board, DstNetName);
                        if Net <> nil then
                        begin
                            ConnectedPrim.BeginModify;
                            ConnectedPrim.Net := Net;
                            ...
                            if (ConnectedPrim.ObjectId = ePolyObject) or
                               ...
                               (ConnectedPrim.ObjectId = eFillObject) then
                            begin
                                if ConnectedPrim.ObjectId = ePolyObject then
                                    ConnectedPrim.Rebuild;
                                Inc(PolygonCount);
                            end;
                        end;
                    end;
        ...
        // Rebuild net connectivity — verified: Net.ConnectivelyInValidate — Distribute.pas L1858
        // Board.ConnectivelyValidateNets does NOT exist; iterate nets and invalidate each.
        NetIter := Board.BoardIterator_Create;
        NetIter.SetState_FilterAll;
